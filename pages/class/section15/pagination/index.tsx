...
export default function PaginationPage() {
    ...
    return (
        <>
            ...
            <span style={PaginationStyle} onClick={() => onNavigatePage("prev")}>{"<"}</span>
            {
                new Array(10).fill(0).map((_, idx) => (
                    (
                        idx + startPage <= boardsCount &&
                            <span
                                key={idx}
                                style={idx + startPage === currPage ? ActiveStyle : PaginationStyle}
                                onClick={() => onRefetchBoard(idx + startPage)}
                            >
                            {idx + startPage}
                            </span>
                    )
            ...
            <span style={PaginationStyle} onClick={() => onNavigatePage("next")}>{">"}</span>
        </>
    )
}
